...
function printReport(totalData: ITotalWarehouse): string {
  let necessaryThings: string[] = [];
  for (let key in totalData) {
    if (totalData[key as keyof ITotalWarehouse] === "empty") {
      necessaryThings.push(key);
    }
  }
  if (necessaryThings.length !== 0) {
    return `We need this items: ${necessaryThings}`;
  } else {
